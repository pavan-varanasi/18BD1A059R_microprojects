...
app.get('/ventilatordetails',middleware.checkToken,(req,res)=>{
    ...
    var ventilatordetails=db.collection('ventilatorsDetails').find().toArray().then(result=>res.json(result));
});

app.post('/searchventbystatus',middleware.checkToken,(req,res)=>{
    var status=req.body.status;//by status searching ventilators
    console.log(status);
    var ventilatordetails=db.collection('ventilatorsDetails').find({"status":status}).toArray().then(result=>res.json(result));
});

app.post('/searchventbyname',middleware.checkToken,(req,res)=>{
    var name=req.query.name;//by hospital name searching ventilators
    console.log(name);
    var ventilatordetails=db.collection('ventilatorsDetails').find({'name':new RegExp(name,'i')}).toArray().then(result=>res.json(result));
});
...
app.put('/updateventilator',middleware.checkToken,(req,res)=>{
    ...
    db.collection("ventilatorsDetails").updateOne(ventid,newvalues,function(err,result){
